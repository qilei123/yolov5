...
from utils.loss import ComputeLoss,ComputeLossOBB
...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    # Trainloader
    train_loader, dataset = create_dataloader_obb(train_path, imgsz, batch_size // WORLD_SIZE, gs, single_cls,
    ...
    compute_loss = ComputeLossOBB(model)  # init loss class
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        optimizer.zero_grad()
        for i, (imgs, targets, paths, _,obb_targets) in pbar:  # batch -------------------------------------------------------------
            ...
            with amp.autocast(enabled=cuda):
                ...
                #print(pred[0].shape)
                #exit(0)
                loss, loss_items = compute_loss(pred, obb_targets.to(device),targets.to(device))  # loss scaled by batch_size
                if RANK != -1:
            ...
            if RANK in [-1, 0]:
                ...
                pbar.set_description(('%10s' * 2 + '%10.4g' * 5) % (
                    f'{epoch}/{epochs - 1}', mem, *mloss, obb_targets.shape[0], imgs.shape[-1]))
        ...
        if RANK in [-1, 0]:
            ...
            if not noval or final_epoch:  # Calculate mAP
                results, maps, _ = val_obb.run(data_dict,
    ...
    if RANK in [-1, 0]:
        ...
        for f in last, best:
            if f.exists():
                ...
                if f is best:
                    LOGGER.info(f'\nValidating {f}...')
                    results, _, _ = val_obb.run(data_dict,
                                            batch_size=batch_size // WORLD_SIZE * 2,
                                            imgsz=imgsz,
