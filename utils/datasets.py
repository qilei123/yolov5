...
def create_dataloader_obb(path, imgsz, batch_size, stride, single_cls=False, hyp=None, augment=False, cache=False, pad=0.0,
    ...
    with torch_distributed_zero_first(rank):  # init dataset *.cache only once if DDP
        if path.endswith('.json'):
            dataset = LoadImagesAndLabels4OBB(path, imgsz, batch_size,
                                      augment=augment,  # augmentation
                                      hyp=hyp,  # hyperparameters
                                      rect=rect,  # rectangular batches
                                      cache_images=cache,
...
class LoadImagesAndLabels4OBB(LoadImagesAndLabels4COCO):
    ...
    def __getitem__(self, index):
        ...
        show_segs(img,np.array([xyn2xy(x, w=img.shape[1], h=img.shape[0],clip=True, eps=1E-3) for x in segments4]))
        if nl:
