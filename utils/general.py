...
def xywh2xyxy(x):#Here in (xywh), xy is the center of the bounding box
...
def xywh2xyxy1(x):#Here in (xywh), xy is the left-top point of the bounding box
    # Convert nx4 boxes from [x, y, w, h] to [x1, y1, x2, y2] where xy1=top-left, xy2=bottom-right
    y = x.clone() if isinstance(x, torch.Tensor) else np.copy(x)
    y[:, 0] = x[:, 0]  # top left x
    y[:, 1] = x[:, 1]  # top left y
...
def non_max_suppression_obb(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if labels and len(labels[xi]):
            lb = labels[xi]
            v = torch.zeros((len(lb), nc + 6), device=x.device)
            v[:, :5] = lb[:, 1:6]  # box
            v[:, 5] = 1.0  # conf
        ...
        # Box (center x, center y, width, height) to (x1, y1, x2, y2)
        #box = xywh2xyxy(x[:, :4]) #should go to obbox
        box = x[:,:5]
        ...
        boxes = 
        boxes, scores =  x[:, 5]  # boxes (offset by class), scores
        #print(boxes)
        #i = torchvision.ops.nms(boxes, scores, iou_thres)  # NMS
