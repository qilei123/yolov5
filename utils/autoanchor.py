...
def check_anchors_obb(dataset, model, thr=4.0, imgsz=640):
    # Check anchor fit to data, recompute if necessary
    m = model.module.model[-1] if hasattr(model, 'module') else model.model[-1]  # Detect()
    shapes = imgsz * dataset.shapes / dataset.shapes.max(1, keepdims=True)
    scale = np.random.uniform(0.9, 1.1, size=(shapes.shape[0], 1))  # augment scale
...
def kmean_anchors(dataset='./data/coco128.yaml', n=9, img_size=640, thr=4.0, gen=1000, verbose=True):
    ...
    if isinstance(dataset, str):  # *.yaml file
        ...
        if 'obb' in data_dict['train']:
            dataset = LoadImagesAndLabels4COCO(os.path.join(data_dict['path'],data_dict['train']), augment=True, rect=True)
        else:
...
def kmean_anchors_obb(dataset='./data/coco128.yaml', n=9, img_size=640, thr=4.0, gen=1000, verbose=True):
    """ Creates kmeans-evolved anchors from training dataset
